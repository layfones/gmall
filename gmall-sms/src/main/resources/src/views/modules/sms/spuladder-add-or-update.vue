    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/sms/spuladder/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.spuId = data.spuladder.spuId
                this.dataForm.fullCount = data.spuladder.fullCount
                this.dataForm.discount = data.spuladder.discount
                this.dataForm.price = data.spuladder.price
                this.dataForm.addOther = data.spuladder.addOther
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/sms/spuladder/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
