    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/sms/spufullreduction/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.spuId = data.spufullreduction.spuId
                this.dataForm.fullPrice = data.spufullreduction.fullPrice
                this.dataForm.reducePrice = data.spufullreduction.reducePrice
                this.dataForm.addOther = data.spufullreduction.addOther
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/sms/spufullreduction/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
